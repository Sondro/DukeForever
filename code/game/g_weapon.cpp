...
qboolean CheckGauntletAttack( gentity_t *ent ) {
	...
	engine->SV_Trace (&tr, muzzle, vec_zero, vec_zero, end, ent->s.number, MASK_SHOT);
...
void Bullet_Fire (gentity_t *ent, float spread, int damage ) {
	...
	for (i = 0; i < 10; i++) {
		...
		engine->SV_Trace (&tr, muzzle, vec_zero, vec_zero, end, passent, MASK_SHOT);
		if ( tr.surfaceFlags & SURF_NOIMPACT ) {
			return;
		}
...
qboolean ShotgunPellet( vec3_t start, vec3_t end, gentity_t *ent ) {
	...
	VectorSubtract(start, end, tr_dir);
	VectorNormalize(tr_dir);
	AngleVectors(tr_dir, tr_forward, NULL, NULL);

	for (i = 0; i < 10; i++) {
		engine->SV_Trace (&tr, tr_start, vec_zero, vec_zero, tr_end, passent, MASK_SHOT);
		traceEnt = &g_entities[ tr.entityNum ];
...
void ShotgunPattern(int count, float spreadx, float spready, vec3_t origin, vec3_t origin2, int seed, gentity_t *ent ) {
	...
	for ( i = 0 ; i < count ; i++ ) {
		r = Q_crandom( &seed ) * (spreadx * 5000) * 16;
		u = Q_crandom( &seed ) * (spready * 5000) * 16;
...
void weapon_supershotgun_fire (gentity_t *ent) {
	...
	ShotgunPattern(14, 0.14, 0.08, tent->s.pos.trBase, tent->s.origin2, tent->s.eventParm, ent);	
}
...
void weapon_railgun_fire (gentity_t *ent) {
	...
	passent = ent->s.number;
	do {
		engine->SV_Trace (&trace, muzzle, vec_zero, vec_zero, end, passent, MASK_SHOT );
		...
		engine->SV_UnlinkEntity( traceEnt );
	...
	for ( i = 0 ; i < unlinked ; i++ ) {
		engine->SV_LinkEntity( unlinkedEntities[i] );
	}
...
void Weapon_Nailgun_Fire(gentity_t* ent, int numNailShots) {
	gentity_t* m;
	int			count;

	for (count = 0; count < numNailShots; count++) {
...
===============
*/
void SetupMuzzlePoint(gentity_t* ent, vec3_t _forward, vec3_t _right, vec3_t _up) {
	CalcMuzzlePoint(ent, forward, right, up, muzzle);
	VectorCopy(_forward, forward);
	VectorCopy(_right, right);
	VectorCopy(_up, up);
}
...
static void KamikazeRadiusDamage( vec3_t origin, gentity_t *attacker, float damage, float radius ) {
	...
	numListedEntities = engine->SV_AreaEntities( mins, maxs, entityList, MAX_GENTITIES );
...
void G_StartKamikaze( gentity_t *ent ) {
	...
	engine->SV_LinkEntity( explosion );
