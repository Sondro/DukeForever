...
qboolean OnSameTeam(gentity_t* target, gentity_t* attacker) {
	return qfalse;
}
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	contents = engine->SV_PointContents( self->r.currentOrigin, -1 );
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
	...
	if (inflictor && inflictor->client && inflictor->client->ps.powerups[PW_QUAD]) {
		G_AddEvent(inflictor, EV_POWERUP_QUAD, 0);
	...
	if ( targ->s.eType == ET_MOVER && targ->secret == qtrue ) {
	...
	if ( client && attacker->client && targ != attacker && targ->health > 0
			&& targ->s.eType != ET_MISSILE
			&& targ->s.eType != ET_MONSTER
...
qboolean CanDamage (gentity_t *targ, vec3_t origin) {
	...
	engine->SV_Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID);
