	...
	if ( waterlevel == 3 ) {
		...
		if ( ent->client->airOutTime < level.time) {
			...
			if ( ent->health > 0 ) {
				...
				G_Damage (ent, NULL, NULL, vec_zero, vec_zero,
...
void	G_TouchTriggers( gentity_t *ent ) {
	...
	//else if (ent->s.eType == ET_MONSTER) {
	//	if (ent->health <= 0) {
	//		return;
	//	}
	//
...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	//	pm.trace = (void(__cdecl*)(trace_t*, const vec_t[], vec_t[], vec_t[], const vec_t[], int, int))engine->SV_Trace;
...
//	qboolean	fired;
	...
	for ( i = oldEventSequence ; i < client->ps.eventSequence ; i++ ) {
		...
		case EV_FALL_FAR:
			...
			G_Damage (ent, NULL, NULL, vec_zero, vec_zero, damage, 0, MOD_FALLING);
			break;
...
//		G_Printf("serverTime >>>>>\n" );
	...
	pm.trace = (void(__cdecl*)(trace_t*, const vec_t[], const vec_t[], const vec_t[], const vec_t[], int, int))engine->SV_Trace;
	pm.pointcontents = engine->SV_PointContents;
...
void ClientThink( int clientNum ) {
	...
	if (!visible(ent, PlayerTrail_LastSpot())) {
		PlayerTrail_Add(ent->r.currentOrigin);
	}
}
