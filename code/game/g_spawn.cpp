...
field_t fields[] = {
	...
	{"script", FOFS(scriptname), F_LSTRING},
	{"message", FOFS(message), F_LSTRING},
	{"map", FOFS(nextMapName), F_LSTRING},
...
void SP_trigger_onlyregistered(gentity_t* ent);

void SP_trigger_changelevel(gentity_t* ent);
...
spawn_t	spawns[] = {
	...
	{"info_teleport_destination", SP_info_camp},
	...
	{"func_door_secret", SP_func_secret_door},
	...
	{ "ambient_suck_wind", ambient_suck_wind},
	{ "ambient_drone", ambient_drone},
	{ "ambient_flouro_buzz", ambient_flouro_buzz},
	{ "ambient_drip", ambient_drip},
	{ "ambient_comp_hum", ambient_comp_hum},
	{ "ambient_thunder", ambient_thunder},
	{ "ambient_light_buzz", ambient_light_buzz},
	{ "ambient_swamp1", ambient_swamp1},
	{ "ambient_swamp2", ambient_swamp2},

	{ "func_bossgate", SP_func_bossgate},

	{"item_shells", SP_item_shells},
	{"item_nails", SP_item_nails},

	{"item_health", SP_health},	
	...
	{ "monster_ogre_marksman", monster_ogre_marksman },
...
void G_SpawnGEntityFromSpawnVars( void ) {
	...
	if ((level.current_skill == 0 && ((int)ent->spawnflags & SPAWNFLAG_NOT_EASY))
		|| (level.current_skill == 1 && ((int)ent->spawnflags & SPAWNFLAG_NOT_MEDIUM))
		|| (level.current_skill >= 2 && ((int)ent->spawnflags & SPAWNFLAG_NOT_HARD)))
	{
		G_FreeEntity(ent);
		return;
	}

	if (g_nomonsters.integer == 1)
	{
...
qboolean G_ParseSpawnVars( void ) {
	...
	if ( !engine->SV_GetEntityToken( com_token, sizeof( com_token ) ) ) {
		// end of spawn string
		return qfalse;
	}
	if ( com_token[0] != '{' ) {
	...
	while ( 1 ) {	
		// parse key
		if ( !engine->SV_GetEntityToken( keyname, sizeof( keyname ) ) ) {
...
void G_SpawnEntitiesFromString( void ) {
	...
	level.sound_zombie_hit = G_SoundIndex("sound/zombie/z_hit.wav");
	level.sound_zombie_miss = G_SoundIndex("sound/zombie/z_miss.wav");
