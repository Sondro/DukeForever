...
void GL_InitRaytracing(int width, int height) {
	...
	tr_create_texture_2d(renderer, width, height, tr_sample_count_1, tr_format_r16g16b16a16_unorm, 1, NULL, false, tr_texture_usage_sampled_image | tr_texture_usage_storage_image, &lightTexture);
...
{
	GL_FindMegaTile(skyTexture, sky_map_x, sky_map_y, sky_map_w, sky_map_h);
...
void GL_CalcFov(float base_fov, float& fov_x, float& fov_y) {
	...
	// 16:9
	ratio_x = r_fovScaleX->integer;
	ratio_y = r_fovScaleY->integer;
...
void GL_Render(float x, float y, float z, vec3_t viewaxis[3])
{
	...
	//else {
		frameData[8] = 1500.0f;
		frameData[9] = 800.0f;
		frameData[10] = tr.fogInfo.fogType;
	//}
