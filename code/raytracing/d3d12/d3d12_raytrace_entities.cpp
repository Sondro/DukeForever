...
void GL_CreateTopLevelAccelerationStructs(bool forceUpdate) {
	...
	if(!onlyUpdate || forceUpdate)
	{
		...
		int numWorldModels = 1;

		// Add in the alpha BSP bits.
		...
		for (int i = 0; i < tr.dxr_refdef.num_entities; i++)
		{
			...
			case MOD_BRUSH:
			case MOD_MESH:
				if (!currententity->e.skipShadows)
				{
					m_topLevelASGenerator.AddInstance(mesh->buffers.pResult.Get(), (DirectX::XMMATRIX&)currententity->dxrTransform, i + numWorldModels, 0xFF, false);
				}
				else
				{
					m_topLevelASGenerator.AddInstance(mesh->buffers.pResult.Get(), (DirectX::XMMATRIX&)currententity->dxrTransform, i + numWorldModels, 0x20, false);
				}
				break;
			}
		}
