...
typedef struct {
	...
	float		color2D[4];		// jmarshall: changed to normalized values.
...
// cvars
//
extern cvar_t* r_fovScaleX;
extern cvar_t* r_fovScaleY;
...
void RE_SetFogParams(fogInfo_t* fogInfo);
...
int RE_GetNumWorldShaders(void);
qhandle_t RE_GetWorldShader(int index);
...
worldMaterial_t* R_FindWorldMaterial(const char* name);
