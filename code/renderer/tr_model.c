...
static qhandle_t globalCustomShader = -1;
...
int RE_ModelNumFrames(qhandle_t handle) {
	return tr.models[handle]->md3[0]->numFrames;
}
...
qhandle_t RE_RegisterModelEx(const char* name, const char* _fileName, qhandle_t globalShader) {
	...
	globalCustomShader = globalShader;
	...
	if (ident == MD4_IDENT) {
		loaded = R_LoadMD4(mod, buf, _fileName);
	}
	else {
		if (ident != MD3_IDENT) {
...
qhandle_t RE_RegisterModel( const char *name ) {
	return RE_RegisterModelEx(name, name, -1);
}
