...
void R_Register( void ) 
{
  ...
  // Default to using SMP on Mac OS X or Linux if we have multiple processors
	...
	r_fovScaleX = ri.Cvar_Get("r_fovScaleX", "16", CVAR_ARCHIVE);
	r_fovScaleY = ri.Cvar_Get("r_fovScaleY", "9", CVAR_ARCHIVE);
...
refexport_t *GetRefAPI ( int apiVersion, refimport_t *rimp ) {
	...
	re.ModelNumFrames = RE_ModelNumFrames;

	re.GetWorldShader = RE_GetWorldShader;
	re.GetNumWorldShaders = RE_GetNumWorldShaders;

	re.RE_RegisterModelEx = RE_RegisterModelEx;

	re.SetFogParams = RE_SetFogParams;
