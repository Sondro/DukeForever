...
#include "cg_local.h"
#include "../../duke4/ui/menudef.h" // bk001205 - for Q3_ui as well
...
void CG_ParseServerinfo( void ) {
	...
	engine->Cvar_Set("g_gametype", va("%i", cgs.gametype));
	...
	engine->Cvar_Set("g_redTeam", cgs.redTeam);
	Q_strncpyz( cgs.blueTeam, Info_ValueForKey( info, "g_blueTeam" ), sizeof(cgs.blueTeam) );
	engine->Cvar_Set("g_blueTeam", cgs.blueTeam);
}
...
static void CG_ParseWarmup( void ) {
	...
	if ( warmup == 0 && cg.warmup ) {
		...
		if (cgs.gametype >= GT_CTF && cgs.gametype <= GT_HARVESTER) {
			engine->S_StartLocalSound( cgs.media.countPrepareTeamSound, CHAN_ANNOUNCER );
		} else
#endif
		{
			engine->S_StartLocalSound( cgs.media.countPrepareSound, CHAN_ANNOUNCER );
		}
	}
	...
	while (o && *o) {
		...
		if (n && *n) {
			...
			if (o) {
				...
				timeOffset[o-t] = 0;
				o++;
				engine->renderer->RemapShader( originalShader, newShader, timeOffset );
			}
		} else {
			break;
		}
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
...
#ifdef MISSIONPACK
		engine->S_StartLocalSound( cgs.media.voteNow, CHAN_ANNOUNCER );
#endif //MISSIONPACK
		...
		cgs.gameModels[ num-CS_MODELS ] = engine->renderer->RegisterModel( str );
	} else if ( num >= CS_SOUNDS && num < CS_SOUNDS+MAX_MODELS ) {
		if ( str[0] != '*' ) {	// player specific sounds don't register here
			cgs.gameSounds[ num-CS_SOUNDS] = engine->S_RegisterSound( str );
		}
...
static void CG_MapRestart( void ) {
	...
	if ( cg.warmup == 0 /* && cgs.gametype == GT_TOURNAMENT */) {
		engine->S_StartLocalSound( cgs.media.countFightSound, CHAN_ANNOUNCER );
		CG_CenterPrint( "FIGHT!", 120, GIANTCHAR_WIDTH*2 );
	}
#ifdef MISSIONPACK
	if (cg_singlePlayerActive.integer) {
		engine->Cvar_Set("ui_matchStartTime", va("%i", cg.time));
		if (cg_recordSPDemo.integer && cg_recordSPDemoName.string && *cg_recordSPDemoName.string) {
			engine->Cbuf_AddText(va("set g_synchronousclients 1 ; record %s \n", cg_recordSPDemoName.string));
		}
	}
#endif
...
int CG_ParseVoiceChats( const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats ) {
	...
	if ( !f ) {
		engine->Com_Printf( va( S_COLOR_RED "voice chat file not found: %s\n", filename ) );
		return qfalse;
	}
	if ( len >= MAX_VOICEFILESIZE ) {
		engine->Com_Printf( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i", filename, len, MAX_VOICEFILESIZE ) );
	...
	if (!Q_stricmp(token, "female")) {
		...
		engine->Com_Printf( va( S_COLOR_RED "expected gender not found in voice chat file: %s\n", filename ) );
		return qfalse;
	}
	...
	while ( 1 ) {
		...
		token = COM_ParseExt(p, qtrue);
		if (Q_stricmp(token, "{")) {
			engine->Com_Printf( va( S_COLOR_RED "expected { found %s in voice chat file: %s\n", token, filename ) );
			return qfalse;
		}
		...
		while(1) {
			...
			if (!Q_stricmp(token, "}"))
				break;
			sound = engine->S_RegisterSound( token );
...
void CG_LoadVoiceChats( void ) {
	...
	size = engine->Hunk_MemoryRemaining();
...
int CG_HeadModelVoiceChats( char *filename ) {
	...
	len = engine->FS_FOpenFileByMode( filename, &f, FS_READ );
	if ( !f ) {
		//engine->Com_Printf( va( "voice chat file not found: %s\n", filename ) );
		return -1;
	}
	if ( len >= MAX_VOICEFILESIZE ) {
		engine->Com_Printf( va( S_COLOR_RED "voice chat file too large: %s is %i, max allowed is %i", filename, len, MAX_VOICEFILESIZE ) );
...
void CG_PlayVoiceChat( bufferedVoiceChat_t *vchat ) {
	...
	if ( !cg_noVoiceChats.integer ) {
		engine->S_StartLocalSound( vchat->snd, CHAN_VOICE);
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "cp" ) ) {
		CG_CenterPrint( CG_Argv(1), SCREEN_HEIGHT * 0.50, BIGCHAR_WIDTH );
		return;
	}

	if (!strcmp(cmd, "wp")) {
		int weaponnum = atoi(CG_Argv(1));
	...
	if ( !strcmp( cmd, "print" ) ) {
		...
		if ( !Q_stricmpn( cmd, "vote failed", 11 ) || !Q_stricmpn( cmd, "team vote failed", 16 )) {
			engine->S_StartLocalSound( cgs.media.voteFailed, CHAN_ANNOUNCER );
		} else if ( !Q_stricmpn( cmd, "vote passed", 11 ) || !Q_stricmpn( cmd, "team vote passed", 16 ) ) {
			engine->S_StartLocalSound( cgs.media.votePassed, CHAN_ANNOUNCER );
		}
#endif
		return;
	}
	...
	if ( !strcmp( cmd, "chat" ) ) {
		if ( !cg_teamChatsOnly.integer ) {
			engine->S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );
  ...
  if ( Q_stricmp (cmd, "remapShader") == 0 ) {
		if (engine->Cmd_Argc() == 4) {
			engine->renderer->RemapShader(CG_Argv(1), CG_Argv(2), CG_Argv(3));
		}
	}
	...
	if (!strcmp(cmd, "sound")) {
		vec3_t origin;
		int soundId = atoi(CG_Argv(1));
		
		origin[0] = atof(CG_Argv(2));
		origin[1] = atof(CG_Argv(3));
		origin[2] = atof(CG_Argv(4));

		engine->S_StartSound(origin, ENTITYNUM_NONE, CHAN_BODY, cgs.gameSounds[soundId]);

		return;
	}

	if (!strcmp(cmd, "lsound")) {
		vec3_t origin;
		int soundId = atoi(CG_Argv(1));

		engine->S_StartLocalSound(soundId, CHAN_BODY);
		return;
	}
...
void CG_ExecuteNewServerCommands( int latestSequence ) {
	while ( cgs.serverCommandSequence < latestSequence ) {
		if ( engine->CL_GetServerCommand( ++cgs.serverCommandSequence ) ) {
			CG_ServerCommand();
		}
	}
}
