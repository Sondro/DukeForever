...
HitInfo FirePrimaryRay() {
  ...
  gLightOutput[launchIndex] += float4(payload.lightColor.rgb, payload.colorAndDistance.a);
...
float getFogFactor(float d, float FogMax, float FogMin)
{
...
[shader("raygeneration")] void RayGen() {
	...
	{		 
		  ...
		  ray.Origin = hit.worldOrigin + (hit.worldNormal);
		  ray.Direction = hit.worldNormal;
		  ray.TMin = 0;
		  ray.TMax = 100000;
